...
// ============================================
// MercadoPago - Credenciais centralizadas (SaaS)
// Usadas SOMENTE para pagamentos de assinaturas de plano
// Lidas da tabela config_saas (substitui ENV vars)
// ============================================

export async function getMPAccessToken(): Promise<string | null> {
  // Prioridade: tabela config_saas > ENV var (fallback legado)
  try {
    const config = await db.$queryRawUnsafe(
      `SELECT "mpAccessToken" FROM "config_saas" LIMIT 1`
    ) as any[];
    if (config[0]?.mpAccessToken) return config[0].mpAccessToken;
  } catch (e) {
    // Tabela pode nao existir ainda
  }

  // Fallback: ENV var (legado — sera removido futuramente)
...
export async function getMPPublicKey(): Promise<string | null> {
  ...
  try {
    const config = await db.$queryRawUnsafe(
      `SELECT "mpPublicKey" FROM "config_saas" LIMIT 1`
    ) as any[];
    if (config[0]?.mpPublicKey) return config[0].mpPublicKey;
  } catch (e) {
    // Tabela pode nao existir ainda
  }

  // Fallback: ENV var (legado — sera removido futuramente)
  if (process.env.MERCADOPAGO_PUBLIC_KEY) return process.env.MERCADOPAGO_PUBLIC_KEY;
  return null;
}
