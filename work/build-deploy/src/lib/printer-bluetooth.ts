// ============================================================
// Printer Service
// Supports: Web Bluetooth thermal printers (smartphones, desktop)
...
// Check if any printer method is available
export function isAnyPrinterAvailable(): boolean {
...
// Connect to a printer (Bluetooth)
...
// ---- High-level: Print CaixaFacil receipt ----
...
export function generateReceiptText(data: Parameters<typeof buildCaixaFacilReceipt>[0]): string {
  ...
  const isCobranca = data.modoOperacao === 'COBRANCA';
  ...
  // JOGADO, CLIENTE e DEBITOS apenas no modo COBRANCA
  if (isCobranca) {
  ...
  // Receitas/ENTRADAS e Despesas/SAIDAS
  ...
  const labelEntradas = isCobranca ? 'RECEITAS' : 'ENTRADAS';
  const labelSaidas = isCobranca ? 'DESPESAS' : 'SAIDAS';
  const labelFinal = isCobranca ? 'TOTALIZACAO' : 'FECHAMENTO';
  ...
  if (hasReceitas) {
    ...
    text += `${labelEntradas}..: R$ ${totalRec.toFixed(2)}\n`;
  }
  ...
  if (hasDespesas) {
    ...
    text += `${labelSaidas}...: R$ ${totalDesp.toFixed(2)}\n`;
    text += sep + '\n';
  }
  ...
  if (data.liquido !== undefined) {
    ...
    text += `${labelFinal}: R$ ${data.liquido.toFixed(2)} ${suffix}\n`;
  }
