...
export function buildCaixaFacilReceipt(data: {
  ...
  liquido?: number;
  modoOperacao?: 'COBRANCA' | 'LEITURA' | 'AJUSTE';
  width?: PrinterWidth;
}): Uint8Array {
  ...
  // Receitas/Entradas - separador antes
  if (hasReceitas) {
    lines.push({ text: separator(width), align: 'center' });
    ...
    lines.push({ text: `${labelEntradas}..: R$ ${totalRec.toFixed(2).replace('.', ',')}` });
  }

  // Despesas/Saidas - separador depois
  if (hasDespesas) {
    ...
    lines.push({ text: `${labelSaidas}...: R$ ${totalDesp.toFixed(2).replace('.', ',')}` });
    lines.push({ text: separator(width), align: 'center' });
  }

  // FECHAMENTO / TOTALIZACAO
  if (data.liquido !== undefined) {
    lines.push({ text: separator(width), align: 'center' });
    const suffix = data.liquido >= 0 ? '[Sobrou]' : '[Faltou]';
    lines.push({ text: `${labelFinal}: R$ ${data.liquido.toFixed(2).replace('.', ',')} ${suffix}`, bold: true, doubleSize: true });
  }
