    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-28 right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 shadow-lg shadow-amber-500/30 flex items-center justify-center hover:scale-110 transition-transform"
          title="Assistente IA"
        >
      ...
      {open && (
        <div
          className={`fixed z-50 bg-card border border-border shadow-2xl rounded-2xl flex flex-col overflow-hidden transition-all duration-300 ${
            minimized
              ? 'bottom-28 right-6 w-72 h-12'
              : 'bottom-28 right-6 w-[calc(100vw-2rem)] max-w-[400px] h-[min(520px,60vh)]'
          }`}
        >
          {/* Header */}
